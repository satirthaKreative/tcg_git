...
class Payments_pro extends CI_Controller 
{
	...
	function index()
	{
		$this->load->view('frontend/payment/paypal/samples/payments_pro');
	}
	...
	{
		$DPFields = array(
							'paymentaction' => $_POST['Paymentaction'], 						// How you want to obtain payment.  Authorization indidicates the payment is a basic auth subject to settlement with Auth & Capture.  Sale indicates that this is a final sale for which you are requesting payment.  Default is Sale.
		...
		$CCDetails = array(
							...
							'acct' => $_POST['acct'], 								// Required.  Credit card number.  No spaces or punctuation.  
							'expdate' => $_POST['expM'].$_POST['expY'], 							// Required.  Credit card expiration date.  Format is MMYYYY
							'cvv2' => $_POST['CVV2'], 								// Requirements determined by your PayPal account settings.  Security digits for credit card.
		...
		$PayerName = array(
							...
							'firstname' => $_POST['fname'], 							// Payer's first name.  25 char max.
							'middlename' => '', 						// Payer's middle name.  25 char max.
							'lastname' => $_POST['lname'], 							// Payer's last name.  25 char max.
							'suffix' => ''								// Payer's suffix.  12 char max.
						);
						
		$BillingAddress = array(
								'street' => $_POST['Street'], 						// Required.  First street address.
								'street2' => $_POST['Street2'], 						// Second street address.
								'city' => $_POST['City'], 							// Required.  Name of City.
								'state' => $_POST['State'], 							// Required. Name of State or Province.
								'countrycode' => $_POST['CountryCode'], 					// Required.  Country code.
								'zip' => $_POST['ZIP'], 							// Required.  Postal code of payer.
								'phonenum' => $_POST['Contact'], 						// Phone Number of payer.  20 char max.
							);
		...
		$PaymentDetails = array(
								...
								'currencycode' => $_POST['CurrencyCode'], 					// Required.  Three-letter currency code.  Default is USD.
								...
								'desc' => $_POST['Desc'], 							// Description of the order the customer is purchasing.  127 char max.
		...
		$OrderItems = array();
		$Item	 = array(
							'l_name' => $_POST['product_name'], 						// Item Name.  127 char max.
		...
		if(!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK']))
		{
			...
			$this->load->view('frontend/payment/paypal/samples/error',$errors);
		}
		else
		{
			...
			$arrEmp = [
				'orderID' => $data['PayPalResult']['TRANSACTIONID'],
				'amount' => $data['PayPalResult']['AMT'],
				'productInfo' => $data['PayPalResult']['REQUESTDATA']['L_NAME0'],
				'firstname' => $data['PayPalResult']['REQUESTDATA']['FIRSTNAME']." ".$data['PayPalResult']['REQUESTDATA']['LASTNAME'],
				'emailID' => $data['PayPalResult']['REQUESTDATA']['EMAIL'],
				'transactionStatus' => 'success',
				'userID' => $_SESSION['session_data'],
				'cAddress' => $data['PayPalResult']['REQUESTDATA']['STREET'].", ".$data['PayPalResult']['REQUESTDATA']['CITY'].", ".$data['PayPalResult']['REQUESTDATA']['STATE'].", ".$data['PayPalResult']['REQUESTDATA']['COUNTRYCODE'].", ".$data['PayPalResult']['REQUESTDATA']['ZIP'],
				'expMonth' => $_POST['expM'],
				'expYear' => $_POST['expY'],
				'payType' => $data['PayPalResult']['REQUESTDATA']['CREDITCARDTYPE'],
				'ipAddress' => $_SERVER['REMOTE_ADDR'],
				'card_number' => $data['PayPalResult']['REQUESTDATA']['ACCT'],
				'typeCard' => 'paypal'
			];
			...
			// $this->load->view('frontend/payment/paypal/samples/do_direct_payment',$data);
		}
	}
